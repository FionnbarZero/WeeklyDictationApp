...
export function cloudDataToAppState(datasets: Dataset[], scores: DatasetScore[], sessions: CloudSession[], attempts: CloudAttempt[], childId: string) {
  const results: WordResult[] = attempts.map((attempt) => ({ id: attempt.id, childId, datasetId: attempt.sourceDatasetId, datasetDateRange: datasets.find((dataset) => dataset.id === attempt.sourceDatasetId)?.dateRange || 'Unknown date range', wordId: attempt.wordId, grade: datasets.find((dataset) => dataset.id === attempt.sourceDatasetId)?.grade || 'Grade 5', phase: attempt.phase, sessionId: attempt.sessionId, sessionDate: attempt.reviewedAt.slice(0, 10), completedAt: attempt.reviewedAt, correct: attempt.correct, revealMethod: 'timer', scored: attempt.completionStatus === 'complete', completeSourceDatasetReviewed: attempt.completionStatus === 'complete' }))
