...
import { firebaseConfigReady, firebaseSetupMessage, DEFAULT_GRADE, DEFAULT_SCHOOL_YEAR, PRIMARY_LIFECYCLE_WARMUP_TRIALS } from './config'
...
function AuthenticatedApp({ auth, now }: { auth: AuthState; now: AppClock }) {
  ...
  ].filter((choice): choice is PracticeTarget => Boolean(choice && choice.dataset.words.length > 0 && !choice.dataset.isWritingWorkshop))
  const warmupSelection = useMemo(() => selectedChild ? buildWarmupSelection({ datasets: primaryDatasets, results: state.results, childWordStates: state.childWordStates, rotationCycleId: state.rotationCycles[selectedChild.id], childId: selectedChild.id, today: currentDate, targetSize: primaryChoices.length > 0 ? PRIMARY_LIFECYCLE_WARMUP_TRIALS : undefined }) : null, [selectedChild, primaryDatasets, state.results, state.childWordStates, state.rotationCycles, currentDateKey, primaryChoices.length])
  ...
  useEffect(() => {
    ...
    void Promise.all([listDatasets(), listSessions(family.id, selectedChild.id), listScores(family.id, selectedChild.id), getCloudAdaptiveState(family.id, selectedChild.id)]).then(async ([rawDatasets, sessions, scores, adaptiveState]) => {
      ...
      if (cancelled) return
      setState(cloudDataToAppState(datasets, scores, readableSessions, attempts.filter((item) => item.completionStatus === 'complete'), selectedChild.id, adaptiveState))
    }).catch((error) => { if (!cancelled) setCloudError(authErrorMessage(error)) }).finally(() => { if (!cancelled) setDataLoading(false) })
    return () => { cancelled = true }
    ...
    if (auth.user && family && finished.cloudSessionId && selectedChild) {
      ...
      if (cloud) {
        ...
        void Promise.all([
          ...
          saveCloudAdaptiveState(family.id, selectedChild.id, {
            childId: selectedChild.id,
            childWordStates: committed.childWordStates.filter((item) => item.childId === selectedChild.id),
            monthlyRotationScores: committed.monthlyRotationScores.filter((item) => item.childId === selectedChild.id),
            rotationCycleId: committed.rotationCycles[selectedChild.id] || 1,
            updatedAt: completionDate.toISOString(),
          }),
